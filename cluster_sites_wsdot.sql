...
SELECT cid_count,
		...
		cluster_id,
		geom
INTO combo_candidates_final_5_wsdot
FROM
(SELECT cid_counts.cid_count,
 		cid_counts.cid,
 		combo_candidates_clustered_5_wsdot.cluster_id,
 		geom,
		ROW_NUMBER() OVER (PARTITION BY cluster_id ORDER BY cid_count DESC) AS rank
FROM combo_candidates_clustered_5_wsdot
JOIN
(SELECT COUNT(*) AS cid_count, cid
FROM combo_candidates_clustered_5_wsdot
GROUP BY cid) AS cid_counts
ON combo_candidates_clustered_5_wsdot.cid = cid_counts.cid) AS ranked_cids
WHERE rank = 1
ORDER BY cid DESC
